<template>
	<view class="">
		...
		<u-popup :show="upshow" @open="open" mode="center" class="up-pop" :round="10">
			<view class="p-3 up-pop-warp rounded-lg">
				<view class="up-title mb-2">
					更新包下载中
				</view>
				<view class="main">
					<u-line-progress :percentage="progress" activeColor="#2979ff"></u-line-progress>
				</view>
			</view>
		</u-popup>
		<view class="pt-5 mt-3">
			...
			<view class="wallet mt-5">
				<view class="wallet-box choose-wallet p-3">
					<view class="title">
						{{this.$t('home.title1')}}
					</view>
					<view class="item flex-between px-2 py-3 rounded-lg mt-2" v-for="(item,index) in defaulList" :key="index" @click="godetail(item)" :class="item.is_type==1?'pun-bg':'bsc-bg'">
...
import {mapState,mapMutations} from 'vuex';
	export default {
		...
		methods:{
			...mapMutations(['saveToken','saveWallet','saveDefault']),
			open(){
				this.upshow=true;
			},
			...
			gocreat(){
					uni.navigateTo({
						url:"/pages/wallet/createWallet/createWallet"
					})
			},
			godetail(item){
				uni.navigateTo({
			...
			//获取默认钱包
			getDefaul(){
				console.log(uni.getStorageSync('wallet_list'))
				if(uni.getStorageSync('wallet_list')){
					this.pageShow=true
					this.defaulList=uni.getStorageSync('wallet_list');
				}
			},
			getWalletList(){
				console.log(uni.getStorageSync('wallet_list'))
				...
				this.$api.getWalletList(this.signForm).then(res=>{
					...
					this.saveWallet(this.list)
					this.saveDefault(this.defaultwallet[0])
					this.defaulList=this.list;
					// this.getDefaul();
					// this.$forceUpdate()
				});
			},
