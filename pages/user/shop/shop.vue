<template>
	<view class="">
		<u-navbar leftText="" :autoBack="true" :safeAreaInsetTop="true" :title="this.$t('mining.title2')" >
			<view class="u-nav-slot" slot="right" >
				<view class="more" @click="goRecord()">
					{{this.$t('mining.title3')}}
				</view>
			</view>
		</u-navbar>
		<view class="p-3 shop">
			...
			<u--form labelPosition="top" :model="form" :rules="rules" ref="form1">
				...
				<u-form-item :label="this.$t('shop.number')" prop="node_number" :borderBottom="false" ref="item1" labelWidth="100%">
					<u--input class="shop-input" v-model="form.node_number" :placeholder="this.$t('shop.number_place')" type="number" clearable @blur="computePun()"></u--input>
				</u-form-item>
				...
				<u-form-item :label="this.$t('shop.pass')" prop="wallet_pass" :borderBottom="false" ref="item1" labelWidth="100%">
					<u--input class="shop-input" v-model="password" :placeholder="this.$t('shop.pass_place')" type="password" clearable></u--input>
				</u-form-item>
			</u--form>
...
export default {
	...
	methods: {
		...
		computePun(){
			this.destroy=this.result.node/this.result.pun*this.form.node_number;
		},
		pickerShow(){
			this.show=true;
		},
		...
		submit(){
			...
			this.$refs.form1.validate().then(res => {
				...
				this.form.wallet_pass=this.$md5.hex_md5(this.password);
				let data=JSON.stringify(Object.assign(this.form,{}));
				console.log(data)
				this.formSign.sign=this.$rsaEncryptlong(data);
				uni.$u.toast('创世节点创建中……')
				this.$api.buyNode(this.formSign).then(res=>{
					console.log(res)
					uni.navigateTo({
						url:"/pages/user/shop/record"
					})
