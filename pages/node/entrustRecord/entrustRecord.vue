<template>
	<view class="p-3">
		<view class="item rounded-lg mb-2" v-for="(item,index) in list" :key="index">
			<view class="time p-2">
				<text>{{$t('myEntrust.timeEntrust')}}：{{item.create_time}}</text>
			</view>
			<view class="main flex p-2 flex-center">
				<view class="box flex-5">
					...
					<view class="title">
						{{$t('myEntrust.numEntrust')}}
					</view>
				</view>
				<view class="box flex-3">
					<view class="statue" :class="item.is_ep==1?'text-blue':'text-green'">
						{{item.is_ep==1 ? $t('myEntrust.state1'):$t('myEntrust.state2')}}
					</view>
					<view class="title">
						{{$t('myEntrust.stateEntrust')}}
					</view>
				</view>
				<view class="box flex-1">
					<u-button type="primary" size="small" :text="$t('myEntrust.ransom')" :disabled="item.is_ep==2" :plain="item.is_lift==1" @click="open(item.transfer_hash)"></u-button>
				</view>
			</view>
		...
		<u-popup :show="show" :round="10" mode="center" :closeOnClickOverlay="false" @close="close" @open="open">
			<view class="pop-warp p-2">
				...
				<view class="pop-btn flex flex-center">
					...
					<u-button :text="this.$t('public.submit')" throttleTime="3000"  type="primary" class="ml-2" @click="relieve"></u-button>
				</view>
			</view>
		</u-popup>
	</view>
	...
	export default {
		...
		onShow() {
			uni.setNavigationBarTitle({
				title: this.$t('myEntrust.entrustRecord')
			});
			this.getList2()
		},
		methods:{
			...
			getList(){
				...
				this.$api.entrustRecord(this.formSign).then(res=>{
					console.log(res)
					let list=res.list
			...
			relieve(){
				...
				this.$api.liftEntrust(this.relieveSign).then(res=>{
					console.log(res)
					uni.$u.toast('赎回成功!')
					this.show=false
					setTimeout(()=>{
